-- Keep Disha Action Hub columns/RPC safe and align server validation with the product workflow.
BEGIN;
...
BEGIN
  ...
  IF normalized_status = 'In Progress'
     AND nullif(btrim(concat_ws('', p_root_cause, p_corrective_action_plan, p_preventive_action_plan, p_action_taken, p_closure_remarks)), '') IS NULL THEN
    RAISE EXCEPTION 'Enter at least one action field before saving In Progress.';
  END IF;

  IF normalized_status = 'Completed' THEN
    IF nullif(btrim(coalesce(p_root_cause, '')), '') IS NULL THEN
      RAISE EXCEPTION 'Root Cause is required before completion.';
    END IF;
